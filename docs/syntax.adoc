...
function Func1@0(){
  ...
  // Scope ID 1 resumes
  // c@1 is still 3 here, since c@2 is a different variable in a different scope.
  // c@1 is stored in the local hash table, since it is used in Func2.
  var d@1 = 4;
  function Func2@1(){
    // Scope ID 3 begins, but `let` and `const` variables are stored in
    // VM registers here, because no nested function or closure is created 
    // inside Func2.
    let e = 5;
    {
      // Scope ID 4 begins, but still in VM registers.
      let e = 6;  
      puts(e); // prints 6
      // Scope ID 4 ends
    }
