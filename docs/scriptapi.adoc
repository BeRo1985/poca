...
[source,js]
----
let old = GarbageCollector.setIncrementalCollectionThresholdFactor(0.125);
----
...
[source,js]
----
// Set the incremental collection threshold factor to 0.125.
let oldFactor = GarbageCollector.setIncrementalCollectionThresholdFactor(0.125);
...
puts("Old incremental collection threshold factor: ", oldFactor);
----
* *Note:* The default value is `0.125`, which means that the GC will
trigger an incremental collection when the number of used objects
exceeds 12.5% of the total allocated objects. A value of `0.5` would
